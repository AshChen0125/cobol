       *>會把單字切斷
       ...
       FILE-CONTROL.
           ...
           SELECT OUT-FILE ASSIGN TO 'test_0923.txt'
               ORGANIZATION IS LINE SEQUENTIAL.
                       ...
                       *> KEYWORD 拆段
                       PERFORM UNTIL FUNCTION LENGTH(FUNCTION TRIM(
                        REMAINING)) = 0
                           ...
                           *> TOKEN ≤ 35 且含 KEYWORD → 直接輸出
                           IF FUNCTION LENGTH(FUNCTION TRIM(TOKEN)) <= 
                           35 AND FOUND = 'Y'
                               MOVE TOKEN TO FIELD-FILL
                               IF FUNCTION TRIM(FIELD-FILL) NOT = SPACES
                                   MOVE FUNCTION TRIM(FIELD-FILL) 
                                   TO TEMP-FIELD
                                   IF TEMP-FIELD NOT = SPACES
                                       STRING TEMP-FIELD DELIMITED BY 
                                       SIZE
                                              '|' DELIMITED BY SIZE
                                         INTO ADDR-OUTPUT
                               ...
                               PERFORM UNTIL TMP-POS > TOKEN-LEN
                                   ...
                                   IF LOC-FOUND = 'Y'
                                       IF EARLIEST-POS > TMP-POS
                                           ...
                                           IF PART-LEN > 0
                                               MOVE PART(TMP-POS:
                                               PART-LEN) TO FIELD-FILL
                                               IF FUNCTION TRIM(
                                                FIELD-FILL) NOT = SPACES
                                                   MOVE FUNCTION TRIM(
                                                    FIELD-FILL) TO 
                                                    TEMP-FIELD
                                                   IF TEMP-FIELD NOT = 
                                   ...
                                   ELSE
                                       ...
                                       TMP-POS + 1
                                       PERFORM UNTIL PART-LEN <= 0
                                           IF PART-LEN > 35
                                               MOVE PART(TMP-POS:35) TO 
                                               FIELD-FILL
                                               COMPUTE TMP-POS = 
                                               TMP-POS + 35
                                               COMPUTE PART-LEN = 

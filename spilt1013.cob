       ...
       FILE-CONTROL.
           ...
           SELECT OUT-FILE ASSIGN TO 'test_1013.csv' 
               ORGANIZATION IS LINE SEQUENTIAL.
       ...
       *> 拆分後的欄位陣列
       ...
       CHECK-ZIP.
           DISPLAY "TEMP-FIELD: "TEMP-FIELD
           IF FUNCTION LENGTH(TEMP-FIELD) > 0 AND 
           FUNCTION LENGTH(ZIP) = 0
               ...
               *> 用空格拆段落
               PERFORM UNTIL NO-IDX > FUNCTION LENGTH(NO-CHECK-TOKEN)
                   ...
                   ADD 1 TO NO-IDX
               END-PERFORM
               SUBTRACT 1 FROM NO-IDX GIVING NO-CHECK-IDX
       
               *> 檢查哪個段落全是數字
               MOVE SPACES TO TEMP-FIELD
               PERFORM VARYING NO-IDX FROM 1 BY 1 UNTIL NO-IDX > 
               NO-CHECK-IDX
                   IF FUNCTION NUMVAL(NO-FIELD(NO-IDX)) > 0
                      AND FUNCTION LENGTH(FUNCTION TRIM(NO-FIELD(
                        NO-IDX))) =
                          FUNCTION LENGTH(FUNCTION NUMVAL-C(NO-FIELD(
                            NO-IDX)))
               ...
               *> 把剩下的不是純數字段落重新組回 TEMP-FIELD
               ...
               PERFORM VARYING NO-IDX FROM 1 BY 1 UNTIL NO-IDX > 
               NO-CHECK-IDX
                   IF NO-FIELD(NO-IDX) NOT = SPACES
                       IF FUNCTION LENGTH(FUNCTION TRIM(TEMP-FIELD)) = 0
                           MOVE FUNCTION TRIM(NO-FIELD(NO-IDX)) TO 
                           TEMP-FIELD
                       ELSE
                           STRING TEMP-FIELD " " NO-FIELD(NO-IDX)
                                  DELIMITED BY SIZE
                                  INTO TEMP-FIELD
                           END-STRING
                       END-IF
       ...
       CHECK-NO.
           IF FUNCTION LENGTH(FUNCTION TRIM(TEMP-FIELD)) > 0
           AND FUNCTION LENGTH(NUMBER-FILED) = 0
